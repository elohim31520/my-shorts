<template>
	<div class="drawing shadow-primary relative bg-#fff relative" ref="target">
		<div
			v-if="selectedMap.has(data)"
			class="absolute top-5 right-5 w-22 lh-22 text-center rounded-full bg-primary color-#fff font-500 z-5"
		>
			{{ selectedMap.get(data) + 1 }}
		</div>
		<div
			v-else
			class="absolute top-5 right-5 w-22 h-22 rounded-full bg-#6a6e75 bd-#AEAEB1 z-5"
		></div>
		...
		>
			...
			>
				<template v-if="isPost">
					<CdnImage :path="data.avatar" round class="w-20 h-20 mr-4 shrink-0" />
					<p class="text-14 color-#656565 mr-a truncate">{{ data.nickname }}</p>
				</template>
				<template v-else>
...
<script setup>
	...
	const isPost = computed(() => {
		const postId = _get(props, 'data.postId', '')
		return !!postId
	})

<template>
	...
	<div class="ball-list flex-y-center justify-between text-center">
		<template v-for="(item, index) in openCode" :key="index">
			<!-- 為每個號碼生成的元素 -->
			<div v-if="!isLastNumber(index)" class="bg-#f9f6ef rounded-25 h-58.31">
				<div class="relative">
					<SvgIcon size="2.1656rem" :name="`ball-${item.color}`" />
					<span class="ball-list__number">
						{{ _trimStart(item.value, '0') }}
					</span>
				</div>
				<!-- 生肖/五行 -->
				<p class="text-12.425 color-#434343">
					{{ `${item.pet}/${item.ws}` }}
				</p>
			</div>
			<template v-else>
				...
				<!-- 最後一個號碼 -->
				<div class="bg-#f9f6ef rounded-25 h-58.31">
					<div class="relative">
						<SvgIcon size="2.1656rem" :name="`ball-${item.color}`" />
						<span class="ball-list__number">
							{{ _trimStart(item.value, '0') }}
